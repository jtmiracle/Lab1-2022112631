...
public class DirectedTextGraph {
  ...
  public void showAndSaveGraph(String outputImage) {
    ...
    for (DefaultWeightedEdge e : graph.edgeSet()) {
      mxICell cell = adapter.getEdgeToCellMap().get(e);
    ...
    BufferedImage img =
        mxCellRenderer.createBufferedImage(adapter, null, 2, java.awt.Color.WHITE, true, null);
    try {
  ...
  public String queryBridgeWords(String w1, String w2) {
    ...
    return String.format(
        "The bridge words from \"%s\" to \"%s\" are: %s.", w1, w2, String.join(", ", bridges));
  }

  /** 功能4：生成新文本 */
  public String generateNewText(String inputText) {
    String norm =
        inputText.toLowerCase().replaceAll("[^a-z\\s]+", " ").replaceAll("\\s+", " ").trim();
  ...
  /**
   * 计算从 w1 到 w2 的最短路径并返回路径顶点列表与总权重信息。
   * 如果任意输入为空或图中不存在对应顶点，会返回相应提示信息。
   */
  public String calcShortestPath(String w1, String w2) {
    // 校验：起点或终点不能为空
    if (w1 == null || w1.isBlank() || w2 == null || w2.isBlank()) {
      return "起点或终点不能为空，请重新输入！";
    }
    ...
    // 检查图中是否包含这两个顶点
    boolean hasW1 = graph.containsVertex(w1);
    boolean hasW2 = graph.containsVertex(w2);
    if (!hasW1 || !hasW2) {
      if (!hasW1 && !hasW2) {
        return String.format("图中不存在顶点 \"%s\" 和 \"%s\"！", w1, w2);
      } else if (!hasW1) {
        return String.format("图中不存在顶点 \"%s\"！", w1);
      } else {
        return String.format("图中不存在顶点 \"%s\"！", w2);
      }
    }
    ...
    // 如果存在一条最短路径，则返回路径顶点列表和总权重
    if (path != null) {
      return String.format(
              "从 \"%s\" 到 \"%s\" 的最短路径：%s，权重和 = %.0f",
              w1,
              w2,
              path.getVertexList(),
              path.getWeight()
      ...
      // 两顶点在图中但不可达
      return String.format("从 \"%s\" 到 \"%s\" 不存在可达路径。", w1, w2);
    }
  }
  /*public String calcShortestPath(String w1, String w2) {
    w1 = w1.toLowerCase();
    ...
    if (graph.vertexSet().size() <= highlightThreshold) {
      ...
      if (path != null) {
        ...
        return String.format(
            "Shortest path from \"%s\" to \"%s\": %s (len=%.0f)",
  ...
  /** 功能5 辅助：可视化并高亮 w1→w2 的所有最短路径 返回一条摘要信息，并在 Swing 窗口中展示高亮路径，同时导出图片 */
  /*private String calcShortestPathVisual(
      String w1, String w2, DijkstraShortestPath<String, DefaultWeightedEdge> dsp) {
    // 1. 找到单条最短路径长度
    GraphPath<String, DefaultWeightedEdge> best = dsp.getPath(w1, w2);
    if (best == null) {
      JOptionPane.showMessageDialog(null, String.format("No path from \"%s\" to \"%s\".", w1, w2));
      return "";
    }
    double minWeight = best.getWeight();

    // 2. 枚举所有简单路径并筛选出权重 == minWeight 的
    AllDirectedPaths<String, DefaultWeightedEdge> allAlg = new AllDirectedPaths<>(graph);
    List<GraphPath<String, DefaultWeightedEdge>> shortest =
        allAlg.getAllPaths(w1, w2, true, graph.vertexSet().size()).stream()
    ...
    JFrame frame =
        new JFrame(
            String.format("Shortest paths from \"%s\" to \"%s\" (len=%.0f)", w1, w2, minWeight));
    frame.getContentPane().add(new mxGraphComponent(adapter));
    frame.pack();
    frame.setVisible(true);
